...
public class ITextUtil {
    ...
    private static final Logger log = LogManager.getLogger(ITextUtil.class);
    ...
    @Autowired
    public ITextUtil(FileUtil fileUtil, CSVUtil csvUtil, MessageSource messageSource, HttpServletRequest request, Environment env) {
     ...
     *
     * @param record          Contains the record meta data
     * @param study           Contains the study meta data
     * @param project         Contains the project meta data
     * @param primaryContri   Contains the primary contributor
     * @param encrypt         True if the PDF-A should be encrypted, otherwise false.
     * @param withAttachments True, if the codebook and the matrix should be attached to the PDF-A file, otherwise false
     ...
     * @param project       Contains the project meta data
     * @param primaryContri Contains the primary contributor
     * @param document      The document in which the cover sheet is inserted
     */
     ...
     *
     * @param record    Contains the record meta data
     * @param document  The document in which the description is inserted
     * @param font_bold Bold font for creating headlines.
     */
     ...
     *
     * @param var Variable for which the table has to be created
    ...
    private Table createVariableCodeBookTable(final SPSSVarDTO var) {
        ...
        Map<String, String> dwAttributes = csvUtil.sortDwAttributes(var);
        ...
        createAndAddRow(table, false, messageSource.getMessage("dataset.import.report.codebook.itemtext", null, Locale.ENGLISH) + ":", dwAttributes.get("dw_itemtext"));
        ...
        createAndAddRow(table, true, messageSource.getMessage("dataset.import.report.codebook.construct", null, Locale.ENGLISH) + ":", dwAttributes.get("dw_construct"));
        createAndAddRow(table, false, messageSource.getMessage("dataset.import.report.codebook.measocc", null, Locale.ENGLISH) + ":", dwAttributes.get("dw_measocc"));
        createAndAddRow(table, true, messageSource.getMessage("dataset.import.report.codebook.instrument", null, Locale.ENGLISH) + ":", dwAttributes.get("dw_instrument"));
        createAndAddRow(table, false, messageSource.getMessage("dataset.import.report.codebook.filtervar", null, Locale.ENGLISH) + ":",
                dwAttributes.get("dw_filtervar").equals("1") ? messageSource.getMessage("export.boolean.true", null, Locale.ENGLISH)
     ...
     *
     * @param matrix Entire data matrix of the record.
     * @param var    Variable for which the table has to be created
    ...
    private Table createVariableDescriptiveStatistikTable(final List<List<Object>> matrix, final SPSSVarDTO var) {
        ...
        if (matrix != null) {
            ...
            if (statsExp.get()) {
                ...
                uniqueSet.forEach(key -> unlabelCC.set(unlabelCC.get() + (int) freq.getCount(key.trim())));
     ...
     *
     * @param table         Table object to which the row has to be appended
     * @param hasBackground Boolean, to create striped Tables
     * @param content       Varargs for the cell content.
     */
     ...
     *
     * @param record Record object which includes the record data.
     * @param pdf    Current PDF-document
     * @param res    StringBuilder object to save errors that may occur, and return them to the calling function for exception handling.
     * @param matrix True if the data matrix, false if the codebook should be saved into the PDF-A.
    ...
    private PdfFileSpec getRecordCSVAttachment(final RecordDTO record, final PdfADocument pdf, final StringBuilder res, final boolean matrix) {
        ...
        byte[] datamatrix = csvUtil.exportCSV(record, res, matrix);
     ...
     *
     * @param record  Record object which includes the record data.
     * @param dest    Path to (temporary) file storage
     * @param encrypt True if the PDF-A should be encrypted, otherwise false.
     ...
     *
     * @param record Record object which includes the meta informations.
     * @param pdf    Current PDF-document
     */
     ...
     *
     * @param pdf      The current PDF-A document
     * @param document The current PDF-A document
     */
     ...
     *
     * @param value    The value which has to be checked.
     * @param misstype The type of the missing format.
     * @param miss1    Missing value one.
     * @param miss2    Missing value two.
     * @param miss3    Missing value three.
