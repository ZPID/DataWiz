...
public class ODFUtil {
	...
	/**
	 * This function creates the BMBF OTF document for the DPM export On success it returns an byte array, otherwise an exception or an empty byte array!
	 * 
	 * @param ProjectForm,
	 *          which contains all necessary data for the export
	 * @param Locale
	 *          to distinguish whether the export is in English or German. At the moment only as a construct, because the export is currently only implemented in
	 *          German.
	...
	public byte[] createBMBFDoc(final ProjectForm pForm, final Locale locale) throws Exception {
		...
		if (dmp != null && project != null) {
			 ...
			 * 5. die schriftart, welche oben in der Klasse definiert werden 6. hintergrundfarbe der Zelle, normal = null, bei überschriften new Color(190, 190, 190)
			 * für dunkles grau und new Color(210, 210, 210) für etwas helleres <br>
			 * 6. die locale (die ist immer dabei)<br>
			 * <br>
			 * wenn der Wert an der 4. Postion aus den Sprachdateien kommt, dann nutzt du den messageSource. Falls der Text noch nicht in der Datei ist, was bei allen
			 * Texten in der 1. und 2. Spalte, sowie den anderen Texten ausserhalb der Tabllen der fall ist, in der ExportResource_de.properties anlegen
			 * 
			 * das schema ist immer "export.odt.<name des Förderes>.<name der tabelle>.<variablenname>=" bsp: export.odt.BMBF.bifo.th1 wenn die Datei nur eine tabelle
			 * hat, dann kannst du <name der tabelle> weglassen, ABER die bezeichner VOR den = müssen eindeutig sein! doppelte vergabe für zu fehlern! Die Datei
			 * findes du unter src/main/resources/locale - die Bearbeitung _en Datei ist nicht notwengig, da der export vorerst nur auf deutsch ist.
			 * 
			 * Spalte 3 und 4 sollte dir ja aus dem PDF kram bekannt sein. 3 ist immer der vorhande Text aus der Sprachdateien und 4 die Werte aus dmp, oder project
			 * Objekt
			 * 
			 * Du kannst createCell einfach immer untereinander schreiben und musst nicht auf irgendwelche typen achten, da ich die funktion so geschrieben habe, das
			 * Sie erkennt ob der übergebene wert ein text, zahl, boolean oder eine liste ist.
			 * 
			 */
			...
			createCell(table, 0, 8, messageSource.getMessage("export.odt.BMBF.bifo.td10", null, locale), regular, null, locale);
			createCell(table, 1, 8, messageSource.getMessage("export.odt.BMBF.bifo.td11", null, locale), regular, null, locale);
			...
			if (pForm.getPrimaryContributor() != null) {
				if (pForm.getPrimaryContributor().getTitle() != null && !pForm.getPrimaryContributor().getTitle().isEmpty())
					primName.append(pForm.getPrimaryContributor().getTitle()).append(" ");
				if (pForm.getPrimaryContributor().getFirstName() != null && !pForm.getPrimaryContributor().getFirstName().isEmpty())
					primName.append(pForm.getPrimaryContributor().getFirstName()).append(" ");
				if (pForm.getPrimaryContributor().getLastName() != null && !pForm.getPrimaryContributor().getLastName().isEmpty())
					primName.append(pForm.getPrimaryContributor().getLastName());
				if (pForm.getPrimaryContributor().getInstitution() != null && !pForm.getPrimaryContributor().getInstitution().isEmpty())
			...
			createCell(table, 2, 9, messageSource.getMessage("export.odt.no.equivalent", null, locale), regular, new Color(210, 210, 210), locale);
			...
			createCell(table, 2, 10, messageSource.getMessage("export.odt.no.equivalent", null, locale), regular, new Color(210, 210, 210), locale);
			...
			createCell(table, 2, 11, messageSource.getMessage("export.odt.no.equivalent", null, locale), regular, new Color(210, 210, 210), locale);
			...
			createCell(table, 0, 13, messageSource.getMessage("export.odt.BMBF.bifo.td20", null, locale), regular_bold, null, locale);
			createCell(table, 0, 14, messageSource.getMessage("export.odt.BMBF.bifo.td21", null, locale), regular, null, locale);
			createCell(table, 1, 14, messageSource.getMessage("export.odt.BMBF.bifo.td22", null, locale), regular, null, locale);
			createCell(table, 2, 14, messageSource.getMessage("dmp.edit.usedDataTypes", null, locale), regular, null, locale);
			createCell(table, 3, 14, dmp.getUsedDataTypes(), regular, null, locale);
			createCell(table, 0, 15, messageSource.getMessage("export.odt.BMBF.bifo.td23", null, locale), regular, null, locale);
			createCell(table, 1, 15, messageSource.getMessage("export.odt.BMBF.bifo.td24", null, locale), regular, null, locale);
			createCell(table, 2, 15, messageSource.getMessage("dmp.edit.existingData", null, locale), regular, null, locale);
			createCell(table, 3, 15, dmp.getExistingData(), regular, null, locale);
			createCell(table, 2, 16, messageSource.getMessage("dmp.edit.dataCitation", null, locale), regular, null, locale);
			createCell(table, 3, 16, dmp.getDataCitation(), regular, null, locale);
			createCell(table, 2, 17, messageSource.getMessage("dmp.edit.existingDataRelevance", null, locale), regular, null, locale);
			createCell(table, 3, 17, dmp.getExistingDataRelevance(), regular, null, locale);
			createCell(table, 2, 18, messageSource.getMessage("dmp.edit.existingDataIntegration", null, locale), regular, null, locale);
			createCell(table, 3, 18, dmp.getExistingDataIntegration(), regular, null, locale);
			createCell(table, 0, 19, messageSource.getMessage("export.odt.BMBF.bifo.td25", null, locale), regular, null, locale);
			createCell(table, 1, 19, messageSource.getMessage("export.odt.BMBF.bifo.td26", null, locale), regular, null, locale);
			createCell(table, 2, 19, messageSource.getMessage("dmp.edit.reliabilityTraining", null, locale), regular, null, locale);
			createCell(table, 3, 19, dmp.getReliabilityTraining(), regular, null, locale);
			createCell(table, 2, 20, messageSource.getMessage("dmp.edit.multipleMeasurements", null, locale), regular, null, locale);
			createCell(table, 3, 20, dmp.getMultipleMeasurements(), regular, null, locale);
			createCell(table, 2, 21, messageSource.getMessage("dmp.edit.qualitityOther", null, locale), regular, null, locale);
			createCell(table, 3, 21, dmp.getQualitityOther(), regular, null, locale);
			createCell(table, 0, 22, messageSource.getMessage("export.odt.BMBF.bifo.td27", null, locale), regular, null, locale);
			createCell(table, 1, 22, messageSource.getMessage("export.odt.BMBF.bifo.td28", null, locale), regular, null, locale);
			createCell(table, 2, 22, messageSource.getMessage("export.odt.no.equivalent1", null, locale), regular, new Color(210, 210, 210), locale);
			createCell(table, 0, 23, messageSource.getMessage("export.odt.BMBF.bifo.td29", null, locale), regular, null, locale);
			createCell(table, 1, 23, messageSource.getMessage("export.odt.BMBF.bifo.td30", null, locale), regular, null, locale);
			createCell(table, 2, 23, messageSource.getMessage("export.odt.no.equivalent2", null, locale), regular, new Color(210, 210, 210), locale);
			createCell(table, 0, 24, messageSource.getMessage("export.odt.BMBF.bifo.td31", null, locale), regular_bold, null, locale);
			createCell(table, 0, 25, messageSource.getMessage("export.odt.BMBF.bifo.td32", null, locale), regular, null, locale);
			createCell(table, 1, 25, messageSource.getMessage("export.odt.BMBF.bifo.td33", null, locale), regular, null, locale);
			createCell(table, 2, 25, messageSource.getMessage("dmp.edit.storageResponsible", null, locale), regular, null, locale);
			createCell(table, 3, 25, dmp.getStorageResponsible(), regular, null, locale);
			createCell(table, 2, 26, messageSource.getMessage("dmp.edit.storagePlaces", null, locale), regular, null, locale);
			createCell(table, 3, 26, dmp.getStoragePlaces(), regular, null, locale);
			createCell(table, 2, 27, messageSource.getMessage("dmp.edit.storageBackups", null, locale), regular, null, locale);
			createCell(table, 3, 27, dmp.getStorageBackups(), regular, null, locale);
			createCell(table, 2, 28, messageSource.getMessage("dmp.edit.storageExpectedSize", null, locale), regular, null, locale);
			createCell(table, 3, 28, dmp.getStorageExpectedSize(), regular, null, locale);
			createCell(table, 0, 29, messageSource.getMessage("export.odt.BMBF.bifo.td34", null, locale), regular, null, locale);
			createCell(table, 1, 29, messageSource.getMessage("export.odt.BMBF.bifo.td35", null, locale), regular, null, locale);
			createCell(table, 2, 29, messageSource.getMessage("dmp.edit.managementWorkflow", null, locale), regular, null, locale);
			createCell(table, 3, 29, dmp.isManagementWorkflow(), regular, null, locale);
			createCell(table, 2, 30, messageSource.getMessage("dmp.edit.managementWorkflowTxt", null, locale), regular, null, locale);
			createCell(table, 3, 30, dmp.getManagementWorkflowTxt(), regular, null, locale);
			createCell(table, 0, 31, messageSource.getMessage("export.odt.BMBF.bifo.td36", null, locale), regular, null, locale);
			createCell(table, 1, 31, messageSource.getMessage("export.odt.BMBF.bifo.td37", null, locale), regular, null, locale);
			createCell(table, 2, 31, messageSource.getMessage("dmp.edit.fileFormat", null, locale), regular, null, locale);
			createCell(table, 3, 31, dmp.getFileFormat(), regular, null, locale);
			createCell(table, 2, 32, messageSource.getMessage("dmp.edit.storageRequirements", null, locale), regular, null, locale);
			createCell(table, 3, 32, dmp.isStorageRequirements(), regular, null, locale);
			createCell(table, 2, 33, messageSource.getMessage("dmp.edit.storageRequirementsTxt", null, locale), regular, null, locale);
			createCell(table, 3, 33, dmp.getStorageRequirementsTxt(), regular, null, locale);
			createCell(table, 0, 34, messageSource.getMessage("export.odt.BMBF.bifo.td38", null, locale), regular, null, locale);
			createCell(table, 1, 34, messageSource.getMessage("export.odt.BMBF.bifo.td39", null, locale), regular, null, locale);
			createCell(table, 2, 34, messageSource.getMessage("dmp.edit.sensitiveDataIncluded", null, locale), regular, null, locale);
			createCell(table, 3, 34, dmp.isSensitiveDataIncluded(), regular, null, locale);
			createCell(table, 2, 35, messageSource.getMessage("dmp.edit.sensitiveDataIncludedTxt", null, locale), regular, null, locale);
			createCell(table, 3, 35, dmp.getSensitiveDataIncludedTxt(), regular, null, locale);
			createCell(table, 0, 36, messageSource.getMessage("export.odt.BMBF.bifo.td40", null, locale), regular_bold, null, locale);
			createCell(table, 0, 37, messageSource.getMessage("export.odt.BMBF.bifo.td41", null, locale), regular, null, locale);
			createCell(table, 1, 37, messageSource.getMessage("export.odt.BMBF.bifo.td42", null, locale), regular, null, locale);
			createCell(table, 2, 37, messageSource.getMessage("dmp.edit.metaDescription", null, locale), regular, null, locale);
			createCell(table, 3, 37, dmp.getMetaDescription(), regular, null, locale);
			createCell(table, 0, 38, messageSource.getMessage("export.odt.BMBF.bifo.td43", null, locale), regular, null, locale);
			createCell(table, 1, 38, messageSource.getMessage("export.odt.BMBF.bifo.td44", null, locale), regular, null, locale);
			createCell(table, 2, 38, messageSource.getMessage("dmp.edit.metaFramework", null, locale), regular, null, locale);
			createCell(table, 3, 38, dmp.getMetaFramework(), regular, null, locale);
			createCell(table, 2, 39, messageSource.getMessage("dmp.edit.selectedMetaPurposes", null, locale), regular, null, locale);
			createCell(table, 3, 39, dmp.getSelectedMetaPurposes(), regular, null, locale);
			createCell(table, 0, 40, messageSource.getMessage("export.odt.BMBF.bifo.td45", null, locale), regular_bold, null, locale);
			createCell(table, 0, 41, messageSource.getMessage("export.odt.BMBF.bifo.td46", null, locale), regular, null, locale);
			createCell(table, 1, 41, messageSource.getMessage("export.odt.BMBF.bifo.td47", null, locale), regular, null, locale);
			createCell(table, 2, 41, messageSource.getMessage("dmp.edit.frameworkNationality", null, locale), regular, null, locale);
			createCell(table, 3, 41, dmp.getFrameworkNationality(), regular, null, locale);
			createCell(table, 2, 42, messageSource.getMessage("dmp.edit.frameworkNationalityTxt", null, locale), regular, null, locale);
			createCell(table, 3, 42, dmp.getFrameworkNationalityTxt(), regular, null, locale);
			createCell(table, 2, 43, messageSource.getMessage("dmp.edit.externalCopyright", null, locale), regular, null, locale);
			createCell(table, 3, 43, dmp.isExternalCopyright(), regular, null, locale);
			createCell(table, 2, 44, messageSource.getMessage("dmp.edit.externalCopyrightTxt", null, locale), regular, null, locale);
			createCell(table, 3, 44, dmp.getExternalCopyrightTxt(), regular, null, locale);
			createCell(table, 2, 45, messageSource.getMessage("dmp.edit.internalCopyright", null, locale), regular, null, locale);
			createCell(table, 3, 45, dmp.isInternalCopyright(), regular, null, locale);
			createCell(table, 2, 46, messageSource.getMessage("dmp.edit.internalCopyrightTxt", null, locale), regular, null, locale);
			createCell(table, 3, 46, dmp.getInternalCopyrightTxt(), regular, null, locale);
			createCell(table, 0, 47, messageSource.getMessage("export.odt.BMBF.bifo.td48", null, locale), regular, null, locale);
			createCell(table, 1, 47, messageSource.getMessage("export.odt.BMBF.bifo.td49", null, locale), regular, null, locale);
			createCell(table, 2, 47, messageSource.getMessage("dmp.edit.sensitiveData", null, locale), regular, null, locale);
			createCell(table, 3, 47, dmp.getSensitiveData(), regular, null, locale);
			createCell(table, 2, 48, messageSource.getMessage("dmp.edit.dataProtection", null, locale), regular, null, locale);
			createCell(table, 3, 48, dmp.isDataProtection(), regular, null, locale);
			createCell(table, 2, 49, messageSource.getMessage("dmp.edit.protectionRequirements", null, locale), regular, null, locale);
			createCell(table, 3, 49, dmp.getProtectionRequirements(), regular, null, locale);
			createCell(table, 2, 50, messageSource.getMessage("dmp.edit.consentObtained", null, locale), regular, null, locale);
			createCell(table, 3, 50, dmp.isConsentObtained(), regular, null, locale);
			createCell(table, 2, 51, messageSource.getMessage("dmp.edit.consentObtainedTxt", null, locale), regular, null, locale);
			createCell(table, 3, 51, dmp.getConsentObtainedTxt(), regular, null, locale);
			createCell(table, 2, 52, messageSource.getMessage("dmp.edit.sharingConsidered", null, locale), regular, null, locale);
			createCell(table, 3, 52, dmp.isSharingConsidered(), regular, null, locale);
			createCell(table, 2, 53, messageSource.getMessage("dmp.edit.irbApproval", null, locale), regular, null, locale);
			createCell(table, 3, 53, dmp.isIrbApproval(), regular, null, locale);
			createCell(table, 2, 54, messageSource.getMessage("dmp.edit.irbApprovalTxt", null, locale), regular, null, locale);
			createCell(table, 3, 54, dmp.getIrbApprovalTxt(), regular, null, locale);
			createCell(table, 0, 55, messageSource.getMessage("export.odt.BMBF.bifo.td50", null, locale), regular, null, locale);
			createCell(table, 1, 55, messageSource.getMessage("export.odt.BMBF.bifo.td51", null, locale), regular, null, locale);
			createCell(table, 2, 55, messageSource.getMessage("export.odt.no.equivalent", null, locale), regular, new Color(210, 210, 210), locale);
			createCell(table, 0, 56, messageSource.getMessage("export.odt.BMBF.bifo.td52", null, locale), regular, null, locale);
			createCell(table, 0, 57, messageSource.getMessage("export.odt.BMBF.bifo.td53", null, locale), regular, null, locale);
			createCell(table, 1, 57, messageSource.getMessage("export.odt.BMBF.bifo.td54", null, locale), regular, null, locale);
			createCell(table, 2, 57, messageSource.getMessage("dmp.edit.publStrategy", null, locale), regular, null, locale);
			createCell(table, 3, 57, dmp.getPublStrategy(), regular, null, locale);
			createCell(table, 2, 58, messageSource.getMessage("dmp.edit.depositName", null, locale), regular, null, locale);
			createCell(table, 3, 58, dmp.getDepositName(), regular, null, locale);
			createCell(table, 2, 59, messageSource.getMessage("dmp.edit.searchableData", null, locale), regular, null, locale);
			createCell(table, 3, 59, dmp.getSearchableData(), regular, null, locale);
			createCell(table, 2, 60, messageSource.getMessage("dmp.edit.transferTime", null, locale), regular, null, locale);
			createCell(table, 3, 60, dmp.getTransferTime(), regular, null, locale);
			createCell(table, 2, 61, messageSource.getMessage("dmp.edit.accessReasonAuthor", null, locale), regular, null, locale);
			createCell(table, 3, 61, dmp.getAccessReasonAuthor(), regular, null, locale);
			createCell(table, 2, 62, messageSource.getMessage("dmp.edit.noAccessReason", null, locale), regular, null, locale);
			createCell(table, 3, 62, dmp.getNoAccessReason(), regular, null, locale);
			createCell(table, 2, 63, messageSource.getMessage("dmp.edit.initialUsage", null, locale), regular, null, locale);
			createCell(table, 3, 63, dmp.getInitialUsage(), regular, null, locale);
			createCell(table, 2, 64, messageSource.getMessage("dmp.edit.usageRestriction", null, locale), regular, null, locale);
			createCell(table, 3, 64, dmp.getUsageRestriction(), regular, null, locale);
			createCell(table, 2, 65, messageSource.getMessage("dmp.edit.accessCosts", null, locale), regular, null, locale);
			...
			createCell(table, 0, 67, messageSource.getMessage("export.odt.BMBF.bifo.td55", null, locale), regular, null, locale);
			createCell(table, 1, 67, messageSource.getMessage("export.odt.BMBF.bifo.td56", null, locale), regular, null, locale);
			createCell(table, 2, 67, messageSource.getMessage("dmp.edit.expectedUsage", null, locale), regular, null, locale);
			createCell(table, 3, 67, dmp.getExpectedUsage(), regular, null, locale);
			createCell(table, 0, 68, messageSource.getMessage("export.odt.BMBF.bifo.td57", null, locale), regular_bold, null, locale);
			createCell(table, 0, 69, messageSource.getMessage("export.odt.BMBF.bifo.td58", null, locale), regular, null, locale);
			createCell(table, 1, 69, messageSource.getMessage("export.odt.BMBF.bifo.td59", null, locale), regular, null, locale);
			createCell(table, 2, 69, messageSource.getMessage("dmp.edit.dataSelection", null, locale), regular, null, locale);
			createCell(table, 3, 69, dmp.isDataSelection(), regular, null, locale);
			createCell(table, 2, 70, messageSource.getMessage("dmp.edit.selectionTime", null, locale), regular, null, locale);
			createCell(table, 3, 70, dmp.getSelectionTime(), regular, null, locale);
			createCell(table, 2, 71, messageSource.getMessage("dmp.edit.selectionResp", null, locale), regular, null, locale);
			createCell(table, 3, 71, dmp.getSelectionResp(), regular, null, locale);
			createCell(table, 0, 72, messageSource.getMessage("export.odt.BMBF.bifo.td60", null, locale), regular, null, locale);
			createCell(table, 1, 72, messageSource.getMessage("export.odt.BMBF.bifo.td61", null, locale), regular, null, locale);
			createCell(table, 2, 72, messageSource.getMessage("dmp.edit.storageDuration", null, locale), regular, null, locale);
			createCell(table, 3, 72, dmp.getStorageDuration(), regular, null, locale);
			createCell(table, 2, 73, messageSource.getMessage("dmp.edit.storageSuccession", null, locale), regular, null, locale);
			createCell(table, 3, 73, dmp.isStorageSuccession(), regular, null, locale);
			createCell(table, 2, 74, messageSource.getMessage("dmp.edit.storageSuccessionTxt", null, locale), regular, null, locale);
			createCell(table, 3, 74, dmp.getStorageSuccessionTxt(), regular, null, locale);
			createCell(table, 0, 75, messageSource.getMessage("export.odt.BMBF.bifo.td62", null, locale), regular_bold, null, locale);
			createCell(table, 0, 76, messageSource.getMessage("export.odt.BMBF.bifo.td63", null, locale), regular, null, locale);
			createCell(table, 1, 76, messageSource.getMessage("export.odt.BMBF.bifo.td64", null, locale), regular, null, locale);
			createCell(table, 2, 76, messageSource.getMessage("dmp.edit.responsibleUnit", null, locale), regular, null, locale);
			createCell(table, 3, 76, dmp.getResponsibleUnit(), regular, null, locale);
			createCell(table, 2, 77, messageSource.getMessage("dmp.edit.involvedInstitutions", null, locale), regular, null, locale);
			createCell(table, 3, 77, dmp.getInvolvedInstitutions(), regular, null, locale);
			createCell(table, 2, 78, messageSource.getMessage("dmp.edit.involvedInformed", null, locale), regular, null, locale);
			createCell(table, 3, 78, dmp.isInvolvedInformed(), regular, null, locale);
			createCell(table, 2, 79, messageSource.getMessage("dmp.edit.contributionsDefined", null, locale), regular, null, locale);
			createCell(table, 3, 79, dmp.isContributionsDefined(), regular, null, locale);
			createCell(table, 2, 80, messageSource.getMessage("dmp.edit.contributionsDefinedTxt", null, locale), regular, null, locale);
			createCell(table, 3, 80, dmp.getContributionsDefinedTxt(), regular, null, locale);
			createCell(table, 0, 81, messageSource.getMessage("export.odt.BMBF.bifo.td65", null, locale), regular, null, locale);
			createCell(table, 1, 81, messageSource.getMessage("export.odt.BMBF.bifo.td66", null, locale), regular, null, locale);
			createCell(table, 2, 81, messageSource.getMessage("dmp.edit.staffDescription", null, locale), regular, null, locale);
			createCell(table, 3, 81, dmp.isStaffDescription(), regular, null, locale);
			createCell(table, 2, 82, messageSource.getMessage("dmp.edit.staffDescriptionTxt", null, locale), regular, null, locale);
			createCell(table, 3, 82, dmp.getStaffDescriptionTxt(), regular, null, locale);
			createCell(table, 2, 83, messageSource.getMessage("dmp.edit.specificCosts", null, locale), regular, null, locale);
			createCell(table, 3, 83, dmp.getSpecificCosts(), regular, null, locale);
			createCell(table, 2, 84, messageSource.getMessage("dmp.edit.specificCostsTxt", null, locale), regular, null, locale);
			createCell(table, 3, 84, dmp.getSpecificCostsTxt(), regular, null, locale);
			createCell(table, 2, 85, messageSource.getMessage("dmp.edit.bearCost", null, locale), regular, null, locale);
	...
	/**
	 * This function creates the H2020 OTF document for the DPM export On success it returns an byte array, otherwise an exception or an empty byte array!
	 * 
	 * @param ProjectForm,
	 *          which contains all necessary data for the export
	 * @param Locale
	...
	public byte[] createH2020Doc(final ProjectForm pForm, final Locale locale) throws Exception {
		...
		if (dmp != null && project != null) {
			...
			createCell(table, 2, 17, messageSource.getMessage("export.odt.H2020.no.equivalent", null, locale), regular, new Color(210, 210, 210), locale);
	...
	public byte[] createDFGDoc(final ProjectForm pForm, final Locale locale) throws Exception {
		...
		if (dmp != null && project != null) {
			...
			createCell(table, 0, 0, messageSource.getMessage("export.odt.DFG.th1", null, locale), regular_bold, null, locale);
			createCell(table, 1, 0, messageSource.getMessage("export.odt.DFG.th2", null, locale), regular_bold, null, locale);
			createCell(table, 1, 1, messageSource.getMessage("export.odt.DFG.th5", null, locale), regular_bold, null, locale);
			createCell(table, 2, 1, messageSource.getMessage("export.odt.DFG.th6", null, locale), regular_bold, null, locale);
			createCell(table, 0, 2, messageSource.getMessage("export.odt.DFG.td12", null, locale), regular, null, locale);
			...
			par.setFont(regular);
			par.appendHyperlink(messageSource.getMessage("export.odt.DFG.ft2", null, locale), new URI(messageSource.getMessage("export.odt.DFG.ft2l", null, locale)));
			doc.addColumnBreak();
	...
	/**
	 * This function creates the PsychData OTF document for the DPM export On success it returns an byte array, otherwise an exception or an empty byte array!
	 * 
	 * @param ProjectForm,
	 *          which contains all necessary data for the export
	 * @param Locale
	...
	/**
	 * This function creates the PreRegistration OTF document for the DPM export On success it returns an byte array, otherwise an exception or an empty byte
	 * array!
	 * 
	 * @param ProjectForm,
	 *          which contains all necessary data for the export
	...
	/**
	 * This function writes content into a Cell. There it needs the current table and position (colnum, rownum). As content, the function expects an object of
	 * type object and interprets the correct content independently. This currently works with numbers, strings, booleans and lists.
	 * 
	 * @param table
	 ...
	 */
	private void createCell(final Table table, final int col, final int row, final Object content, final Font font, final Color color, final Locale locale) {
		if (content != null) {
			Cell cell = table.getCellByPosition(col, row);
			if (content instanceof Number) {
				cell.setDoubleValue((Double) content);
			} else if (content instanceof Boolean) {
				cell.setStringValue(((boolean) content) ? messageSource.getMessage("gen.yes", null, locale) : messageSource.getMessage("gen.no", null, locale));
				...
				usedTypes.forEach(usedType -> {
					...
					if (usedType instanceof Integer) {
						FormTypesDTO type = FORMTYPES.parallelStream().filter(dt -> (dt.getId() == (Integer) usedType)).findFirst().orElse(null);
						// TODO OTHER
						if (type != null) {
	...
	/**
	 * This function inits the formtypes. The formtypes are static, because the can be used for all instances. At the moment there are no functions to change the
	 * formtypes, therefore it is not necessary to check for updates. If the formtypes are changed directly in the database, the DataWiz application has to be
	 * reloaded, or the tomcat has to be restarted.
	 */
