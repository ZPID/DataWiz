...
public class FileUtil {
     ...
     * @param hash   The file as byte array
     * @return The checksum as String
     */
    public String getFileChecksum(final String digest, final byte[] hash) {
        switch (digest) {
            case "MD5":
                return DigestUtils.md5Hex(hash).toLowerCase();
            case "SHA-1":
                return DigestUtils.sha1Hex(hash).toLowerCase();
            case "SHA-256":
                return DigestUtils.sha256Hex(hash).toLowerCase();
        }
        return "";
    }
    ...
    public FileDTO buildFileDTO(final long projectID, final long studyID, final long recordID, final long version, final long userID,
        ...
        file.setMd5checksum(getFileChecksum("MD5", file.getContent()));
        file.setSha1Checksum(getFileChecksum("SHA-1", file.getContent()));
        file.setSha256Checksum(getFileChecksum("SHA-256", file.getContent()));
