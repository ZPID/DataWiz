 ...
 * <br />
 * Enumeration for SPSS page encoding
 *
 * @author Ronny Boelter
 * @version 1.0
 */
public enum SPSSPageEncoding {

    SPSS_ENCODING_CODEPAGE(0), /* Text encoded in current code page */
    SPSS_ENCODING_UTF8(1), /* Text encoded as UTF-8 */
    SPSS_UNKNOWN(-99);

    private final int number;

    private SPSSPageEncoding(int s) {
        number = s;
    }
    ...
    private static final Map<Integer, SPSSPageEncoding> intToTypeMap = new HashMap<Integer, SPSSPageEncoding>();

    static {
        for (SPSSPageEncoding type : SPSSPageEncoding.values()) {
            intToTypeMap.put(type.number, type);
        }
    }

    public static SPSSPageEncoding fromInt(int i) {
        SPSSPageEncoding type = intToTypeMap.get(Integer.valueOf(i));
        if (type == null)
            return SPSSPageEncoding.SPSS_UNKNOWN;
        return type;
    }
