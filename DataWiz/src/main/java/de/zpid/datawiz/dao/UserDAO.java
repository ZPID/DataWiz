...
public class UserDAO {
	...
	public UserDTO findById(final long id) throws Exception {
		...
		final UserDTO user = this.jdbcTemplate.query("SELECT * FROM dw_user WHERE id= ?", new Object[] { id }, new ResultSetExtractor<UserDTO>() {
			@Override
	 ...
	 * 
	 * @param email
	 *          The primary mail address is defined as unique in the DB, therefore it is possible to search a User by mail.
	 * @param pwd
	...
	public UserDTO findByMail(final String email, final boolean pwd) throws SQLException {
		...
		UserDTO user = this.jdbcTemplate.query("SELECT * FROM dw_user WHERE email= ?", new Object[] { email }, new ResultSetExtractor<UserDTO>() {
			@Override
			public UserDTO extractData(ResultSet rs) throws SQLException, DataAccessException {
				if (rs.next()) {
	...
	public String findPasswordbyId(final long id) throws SQLException {
		...
		final String pwd = this.jdbcTemplate.query("SELECT dw_user.password FROM dw_user WHERE id= ?", new Object[] { id }, new ResultSetExtractor<String>() {
			@Override
	...
	public List<UserDTO> findAll() throws SQLException {
		...
		List<UserDTO> ret = this.jdbcTemplate.query(sql, new Object[] {}, new RowMapper<UserDTO>() {
			public UserDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
				return setUserDTO(false, rs);
			}
		});
