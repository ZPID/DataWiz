          ...
          <table class="table table-striped">
            <thead>
              <tr>
                ...
                <th><s:message code="gen.phone" /></th>
                <th><s:message code="gen.email" /></th>
                <th><s:message code="gen.secEmail" /></th>
                <th><s:message code="gen.institution" /></th>
                <th><s:message code="gen.department" /></th>
            ...
            <tbody>
              <c:forEach items="${userlist}" var="user" varStatus="count">
                <tr>
                  ...
                  <td><s:message text="${user.phone}" /></td>
                  <td><c:if test="${not empty user.email}">
                      <a href="mailto:<s:message text="${user.email}" />" target="_blank"><s:message
                          text="${user.email}" /></a>
                    </c:if></td>
                  <td><c:if test="${not empty user.secEmail}">
                      <a href="mailto:<s:message text="${user.secEmail}" />" target="_blank"><s:message
                          text="${user.secEmail}" /></a>
                    </c:if></td>
                  <td><s:message text="${user.institution}" /></td>
                  <td><s:message text="${user.department}" /></td>
                  <td><s:message text="${user.city}" /></td>
                  <td><s:message text="${user.country}" /></td>
                  <td><c:if test="${not empty user.orcid}">
                      <a href="https://orcid.org/<s:message text="${user.orcid}" />" target="_blank"><s:message
                          text="${user.orcid}" /></a>
                    </c:if></td>
                  <td style="text-align: right;"><a href='<c:url value="/admin/list/project/${user.id}" />'><span
                      class="glyphicon glyphicon-search" aria-hidden="true"></span></a></td>
                  <td style="text-align: right;"><a href="javascript:void(0)" onclick="openDetailModal('user', ${user.id});"> <span
            ...
            <tbody>
              <c:forEach items="${projectlist}" var="project" varStatus="count">
                <tr>
                  ...
                  <td style="text-align: center;"><a href='<c:url value="/project/${project.id}/" />'><span
...
<div id="detailModal" class="modal fade" role="dialog">
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      <form action="<c:url value="/admin/save/user" />" class="form-horizontal" method="post" id="userSubmitForm"
        autocomplete="off">
        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" /> <input type="hidden"
          id="modal_uid" name="modal_uid" />
...
<script>
	...
	function openDetailModal(type, id){		
	    ...
	    success: function(response){
	    	...
	    	$('#modal_uid').val(response.id);
	    	$('#modal_title').val(response.title);
	    	$('#modal_first_name').val(response.firstName);
	    	$('#modal_last_name').val(response.lastName);
	    	$('#modal_email').val(response.email);
	    	$('#modal_sec_email').val(response.secEmail);
	        $('#modal_password').val(response.password);
	        $('#modal_account_state').val(response.account_state);
	      // Display Modal
